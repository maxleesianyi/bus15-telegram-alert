...
function composeLookupMessage(lookup, arrivals, now) {
  const header =
    `Bus ${lookup.serviceNo} at stop ${lookup.busStopCode} checked ${formatSingaporeTime(now)} SGT`;
...
function parseBusLookup(text) {
  const match = text.trim().match(/^\/?bus\s+([a-z0-9]+)\s+(\d{5})$/i);
  if (!match) return null;

  return {
    serviceNo: match[1].toUpperCase(),
    busStopCode: match[2],
  };
}

async function fetchBusArrivals(config, lookup = {}) {
  const url = new URL(LTA_BUS_ARRIVAL_URL);
  url.searchParams.set("BusStopCode", lookup.busStopCode || config.busStopCode);
  url.searchParams.set("ServiceNo", lookup.serviceNo || config.serviceNo);
